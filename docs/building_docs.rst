...
.. note::
   ...
   The first Docker run downloads the TeX Live image (~4GB), which takes several minutes.
...
Some Unicode characters (like δ) may cause LaTeX warnings but won't prevent PDF generation.
The build uses ``-interaction=nonstopmode`` to continue through errors.
...
- `nbsphinx - Jupyter Integration <https://nbsphinx.readthedocs.io/>`_
