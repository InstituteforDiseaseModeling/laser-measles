...
class BaseImportationParams(BaseModel):
    ...
    importation_schedule: Optional[np.ndarray] = Field(
        default=None,
        description="Time-varying importation schedule"
    )
    
    target_patches: Optional[list] = Field(
        default=None,
        description="Patches that receive importations (None = all patches)"
    )
...
class BaseImportation(BasePhase, ABC):
    ...
    def __init__(self, model, verbose: bool = False, params: Optional[BaseImportationParams] = None):
