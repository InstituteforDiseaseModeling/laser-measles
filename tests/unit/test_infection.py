...
def test_infection_single_patch(measles_module):
    ...
    model = MeaslesModel.Model(scenario, MeaslesModel.Params(num_ticks=25, verbose=VERBOSE), name="test_infection_single_patch")
    model.components = [
...
def test_infection_two_patch(measles_module):
    ...
    model = MeaslesModel.Model(scenario, MeaslesModel.Params(num_ticks=25, verbose=VERBOSE), name="test_infection_double_patch")
    model.components = [
        MeaslesModel.components.InfectionSeedingProcess,
        MeaslesModel.components.InfectionProcess]
    model.run()
