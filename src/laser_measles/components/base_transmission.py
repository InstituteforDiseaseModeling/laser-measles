...
class BaseTransmissionParams(BaseModel):
    ...
    beta: float = Field(
        default=0.1, 
        description="Transmission rate parameter", 
        gt=0.0
    )
    ...
    seasonal_amplitude: float = Field(
        default=0.0,
        description="Amplitude of seasonal variation (0.0 = no seasonality)",
        ge=0.0,
        le=1.0
    )
    seasonal_phase: float = Field(
        default=0.0,
        description="Phase offset for seasonal variation (in radians)",
        ge=0.0,
        le=2*np.pi
    )
    
    # Spatial mixing parameters
    mixing_matrix: Optional[np.ndarray] = Field(
        default=None,
        description="Spatial mixing matrix between patches"
    )
    
    # Import/export parameters
    importation_rate: float = Field(
        default=0.0,
        description="Rate of imported infections per time step",
        ge=0.0
    )
...
class BaseTransmission(BasePhase, ABC):
    ...
    def __init__(self, model, verbose: bool = False, params: Optional[BaseTransmissionParams] = None):
    ...
    def get_seasonal_multiplier(self, tick: int, time_scale: str = 'daily') -> float:
        ...
        seasonal_factor = 1.0 + self.params.seasonal_amplitude * np.sin(
            2 * np.pi * t + self.params.seasonal_phase
        )
