...
def debug_plot_age_pyramid(model, age_bins, model_pyramid, wpp_pyramid):
    ...
    plt.bar(age_bins[:-1]/365, model_pyramid / np.sum(model_pyramid),
            width=np.diff(age_bins)/365, align="edge", alpha=0.7, label='Model')
    plt.bar(age_bins[1:-1]/365, wpp_pyramid / np.sum(wpp_pyramid),
            width=np.diff(age_bins[1:])/365, align="edge", alpha=0.7, label='WPP')
