...
custom_infection_params = InfectionParams(
    beta=50.0,
    seasonality=0.1,
    season_start=13
)
...
scenarios = {
    "baseline": {
        ...
        "importation": ImportationPressureParams(),
        "surveillance": CaseSurveillanceParams()
    },
    "high_transmission": {
...
def create_infection_component_params(transmission_rate: float, seasonal_variation: float) -> InfectionParams:
    ...
    return InfectionParams(
        ...
        seasonality=seasonal_variation,
        season_start=0
    )
...
def validate_parameter_ranges(params: InfectionParams) -> bool:
    ...
    return (
        0 < params.beta < 100 and
        0 <= params.seasonality <= 1 and
        0 <= params.season_start <= 25
    )
